...
// Promise factory that resolves to val after the delay
...
// Promise factory that reject after the delay
...
// Promise factory that, after the delay, resolve to val if val >= 0
// else reject
...
describe("callback", () => {
  it("should receive val as parameter", (done) => {
    cbFactory((par) => {
      expect(par).toBe(1);
      done();
    }, 1);
  ...
  it("should not receive a defined parameter", (done) => {
    cbFactory((par) => {
...
describe("promise", () => {
  it("should receive val as parameter when the promise resolve", () => {
    return pmFactory(1).then((par) => {
      expect(par).toBe(1);
    });
  });

  it("should not receive a defined parameter in the promise chain", () => {
    return pmFactory(1)
      .then((par) => {
        expect(par).toBe(1);
      })
  ...
  it("should receive a 1, 2, 3 array when all promises resolve", () => {
    const arr = [pmFactory(1), pmFactory(2), pmFactory(3)];
    return Promise.all(arr).then((par) => {
  ...
  it("should catch the error", () => {
    ...
    return pmErrFactory(2)
      .then((par) => {
        // Line never executed because the Promise is
        // is rejected
        num += par;
      })
      .catch((err) => {
...
describe("awaiting promises", () => {
  it("should receive val when resolve", async () => {
  ...
  it("should receive a 1, 2, 3 array when all promises resolve", async () => {
    const arr = [pmFactory(1), pmFactory(2), pmFactory(3)];
...
describe("testing resolve / rejection with await and try catch", () => {
  it("should resolve to 1 (positive number resolves)", async () => {
    let ret = 0;
    try {
  ...
  it("should reject and catch the error (negative number rejects)", async () => {
    let ret = 0;
    try {
      ret = await pmPositiveFactory(-1);
    } catch (err) {
